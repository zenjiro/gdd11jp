    ...
    public static int h(final long boardConfig) {
        ...
        for (int pos = numOfTiles - 1; pos >= 0; --pos) {
            ...
            if (goalStateTile != 0 && currentTile != goalStateTile) {
                final int
                    currentPosition =
                        Utility.getElementAt(currentPositions, goalStateTile),
                    goalStatePosition =
                        Utility.getElementAt(goalStatePositions, goalStateTile),
                ...
                for (int i = lowerIndex; i < upperIndex; ++i) {
                    for (int j = i + 1; j <= upperIndex; ++j) {
                        final byte iValue = (byte)((boardConfig >> (i << 2)) & 0xF),
                                   jValue = (byte)((boardConfig >> (j << 2)) & 0xF);
                ...
                for (int i = lowerIndex; i <= upperIndex; i += dimension) {
                    for (int j = i + dimension; j <= upperIndex; j += dimension) {
                        ...
                        for (int x = dimMinusOne; x >= 0; --x) {
                            ...
                            if (iValue == val) iFound = true;
                            if (jValue == val) jFound = true;
                        }
